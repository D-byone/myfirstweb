...
class SphinxClient:
	def __init__ (self):
		...
		self._maxmatches	= 20    					    # max matches to retrieve
	...
	def RunQueries (self):
		...
		for i in range(0,nreqs,1):
			...
			result['time'] = '%.3f' % (result['time']/30000000.0)
			p += 16
	...
	def UpdateAttributes ( self, index, attrs, values, mva=False ):
		...
		Example:
			res = cl.UpdateAttributes ( 'test1', [ 'group_id', 'date_added' ], { 2:[123,30000000000000], 4:[456,1234567890] } )
		"""
		assert ( isinstance ( index, str ) )
